...
from utils import get_data_info, shuffle, lrfn, train, egg_spec_augmentation
...
def run(min_votes, augmentation, label_smoothing, data_type, n_epochs):
    ...
    model_info["min_votes"] = min_votes
    model_info["augmentation"] = augmentation
    model_info["label_smoothing"] = label_smoothing
    ...
    model_path = path_out + f"model_{model_name}_{index}.pt"
    while os.path.exists(model_path):
        index += 1

    configs = {
        "n_epochs": n_epochs,
        "path_model_out": model_path,
    ...
    track_loss, track_loss_val = train(
        model, train_loader, valid_loader, valid_loss_min=valid_loss_min, **configs
    )
    ...
    model_info_path = path_out + f"model_info_{model_name}_{index}.json"
...
for mv in min_votes:
    for aug in augmentation:
        for ls in label_smoothing:
            run(mv, aug, ls, data_type, n_epochs)
            gc.collect()
